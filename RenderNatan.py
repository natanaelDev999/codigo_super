...
# procura dados em estruturas de dados
def procura_dados(procurado,local,):
    achado = None
    for pos,v in enumerate(local['variáveis']):
        if v[0] == procurado:
            achado = v[1]
    return achado
...
def compila_codigo_lsn(codigo,pixel,x,y):
    ...
    - ec: termina uma condicional ,depois desta função nenhuma ação sobre $ será executada apenas se houver cs
    VARIÁVEIS :
    - l: cria e salva na memória do pixel uma variável
    VETORES :
    - t: cria um vetor e o salva na memória do pixel
    '''
    global h,w
    # variáveis internas
    pixel_retorna = ' '
    ...
    for c in codigo.strip():
        if c != ';':
            ...
            if linha.startswith('t'):
                # estrutura: comando-' '-nome-' '-valores*
                linha = linha.strip()
                dados = linha.split(' ')
                achou = False
                for c in estruturas_dados['vetores']:
                    if c[0] == dados[1]:
                        achou = True
                if dados[1] != 'p':
                    if achou == False:
            ...
            if linha.startswith('$t'):
                if ativacao_if == True:
                    ...
                    if dados[1] != 'p':
                        if achou == False:
                            estruturas_dados['vetores'].append([dados[1], dados[2:]])
            # inseri um valor novo a x e y do pixel
            if linha.startswith('ot'):
                dados = linha.split(' ')
                for v in estruturas_dados['vetores']:
                    if v[0] == dados[2]:
                        if int(v[1][0]) < w and int(v[1][1]) < y:
                            x = v[1][0]
                            y = v[1][1]
            # soma um vetor a x e y do pixel
            if linha.startswith('ts'):
                dados = linha.split(' ')
                for v in estruturas_dados['vetores']:
                    if v[0] == dados[2]:
                        # eu sei que colocar aqui é algo meio estranho mas resolveu
                        if x + int(v[1][0]) < w and y + int(v[1][1]) < h:
                            x += int(v[1][0])
                            y += int(v[1][1])
                        break
            # subtrai um vetor a x e y do pixel
            if linha.startswith('tb'):
                dados = linha.split(' ')
                for v in estruturas_dados['vetores']:
                    if v[0] == dados[2]:
                        # eu sei que colocar aqui é algo meio estranho mas resolveu
                        if x - int(v[1][0]) > 0 and y - int(v[1][1]) > 0:
                            x -= int(v[1][0])
                            y -= int(v[1][1])
                        break
            linha = ''
...
def main():
    ...
    codigo_lsn = '''
                  ...
                  '''
    #
    # RECOMENDAÇÃO: Rode o código no terminal para melhor desempenho, cuidado ao rodar no Pycharm dependendo da sua configuração
    #
    while True:
