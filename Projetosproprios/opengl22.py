...
def key_callback(window, key, scancode, action, mods):
    global camera_alvo
    if key == glfw.KEY_W:
        camera_alvo[1] += math.sin(math.radians(5))
    elif key == glfw.KEY_S:
        camera_alvo[1] -= math.sin(math.radians(5))
    elif key == glfw.KEY_A:
        camera_alvo[0] -= math.sin(math.radians(5))
    elif key == glfw.KEY_D:
        camera_alvo[0] += math.sin(math.radians(5))
# função inicial
def init():
...
# função para desenho
def render():
    global vao_id, quantidade_vertices, shader_program_id, quantidade_faces,id_uniform_cor_luz,camera_alvo,posicao_camera
