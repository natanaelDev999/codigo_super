...
# camera
posicao_camera = pyrr.Vector3([0.0, 0.0, -3.0])
# quadrado
# vértices
# [2,2, 1,0,0]
# [-2,2, 1,0,0]
...
# função inicial
def init():
    global vertices, vao_id, shader_program_id, faces, id_uniform_projecao, camera_visao,id_uniform_cor_luz,posicao_camera
    ...
    camera_visao = pyrr.matrix44.create_from_translation(posicao_camera)

# função para desenho
def render():
    global vao_id, quantidade_vertices, shader_program_id, quantidade_faces,angulo,camera_visao,id_uniform_cor_luz
    ...
    model_z = pyrr.matrix44.create_from_z_rotation(math.radians(angulo))
    model_y = pyrr.matrix44.create_from_y_rotation(math.radians(angulo))
    model_x = pyrr.matrix44.create_from_x_rotation(math.radians(angulo))

    # multiplição de matrizes
    matrix_completa_projecao = pyrr.matrix44.multiply(model_y, camera_visao)
    matrix_completa_projecao = pyrr.matrix44.multiply(model_x, matrix_completa_projecao)
    matrix_completa_projecao = pyrr.matrix44.multiply(model_z, matrix_completa_projecao)
