...
import ctypes
# biblioteca para matemática matricial e vetorial
import pyrr
...
def normaliza_glm_converte_pyrr(vetor):
    vetor = glm.normalize( glm.vec3(vetor[0],vetor[1],vetor[2]) )
    return pyrr.Vector3([vetor.x,vetor.y,vetor.z])
#####################################################
...
angulo_x = 0
# camera
camera_posicao = pyrr.Vector3([0.0, 0.0, 3.0])
camera_cima = pyrr.Vector3([0.0,1.0,0.0])
pitch = 0
yaw = -90
# vertices
# quadrado
...
def key_callback(window, key, scancode, action, mods):
    global camera_posicao,camera_frente,camera_cima,yaw,pitch
    if key == glfw.KEY_W:
        camera_posicao += 0.05 * camera_frente
    elif key == glfw.KEY_S:
        camera_posicao -= 0.05 * camera_frente
    elif key == glfw.KEY_A:
        camera_posicao -= normaliza_glm_converte_pyrr(pyrr.vector3.cross(camera_frente, camera_cima)) * 0.05
    elif key == glfw.KEY_D:
        camera_posicao += normaliza_glm_converte_pyrr(pyrr.vector3.cross(camera_frente, camera_cima)) * 0.05
    elif key == glfw.KEY_I:
        pitch += 1.5
...
# função inicial
def init():
    global vertices, vao_id, shader_program_id, faces, id_uniform_projecao, camera_visao, id_uniform_cor_luz
    ...
    camera_visao = pyrr.matrix44.create_from_translation(pyrr.Vector3([0.0, 0.0, -3.0]))


# função para desenho
def render():
    global vao_id, quantidade_vertices, shader_program_id, quantidade_faces,camera_visao, id_uniform_cor_luz, angulo_y , angulo_x,camera_posicao,camera_frente,camera_cima
    ...
    direcao = pyrr.Vector3([0.0, 0.0, 0.0])
    direcao[0] = math.cos(math.radians(yaw)) * math.cos(math.radians(pitch))
    direcao[1] = math.sin(math.radians(pitch))
    direcao[2] = math.sin(math.radians(yaw)) * math.cos(math.radians(pitch))

    camera_frente = normaliza_glm_converte_pyrr(direcao)

    camera_frente = normaliza_glm_converte_pyrr(direcao)

    camera_alvo = pyrr.Vector3([0.0, 0.0, 0.0])
    camera_direcao = normaliza_glm_converte_pyrr(camera_posicao - camera_alvo)
    cima_global = pyrr.Vector3([0.0, 1.0, 0.0])
    camera_direita = normaliza_glm_converte_pyrr(pyrr.vector3.cross(cima_global, camera_direcao))
    cima_camera = pyrr.vector3.cross(camera_direcao, camera_direita)

    matriz_LookAt = pyrr.matrix44.create_look_at(eye=camera_posicao, target=camera_posicao + camera_frente,
                                                 up=camera_cima)

    aspect_ratio = 600 / 600
    # matrizes
    projecao = pyrr.matrix44.create_perspective_projection_matrix(45.0, aspect_ratio, 0.1, 100.0)
    model_y = pyrr.matrix44.create_from_x_rotation(math.radians(angulo_x))
    model_x = pyrr.matrix44.create_from_y_rotation(math.radians(angulo_y))

    # multiplição de matrizes
    matrix_completa_projecao = pyrr.matrix44.multiply(model_y, matriz_LookAt)
    matrix_completa_projecao = pyrr.matrix44.multiply(model_x, matrix_completa_projecao)
    matrix_completa_projecao = pyrr.matrix44.multiply(matrix_completa_projecao, projecao)
